...
Paper 1 研究中文版本化知识库中的隐蔽事实污染，当前范围是 Benchmark、Detection、Risk Score、Signals 与 Explanation。S6.1-LR1 已人工验收；R0 带阻塞项验收；FU1 的 P0、L1 已验收。W2 第一次运行是被模型下载阻塞的有效工程运行，Correction 02 已补齐证据闭环；H1 离线模型包已由本机准备并验证，正等待 5090 独立验证。下一步仅能在新审批门后继续 W2。当前没有正式论文实验结果。最大工程风险是 5090 离线加载与 detection-core 尚未实证；最大论文风险是外部基线尚未形成统一严格比较，“多视角解毒”技术范围仍待确认。
...
| 当前阶段 | S6.1-R0-FU1 |
| 当前任务 | 文档体系重构；实验侧等待 H1 的 5090 验证审批与执行 |
...
| S6.1-R0-FU1 | 定向解除基线阻塞 | `IN_PROGRESS / 5090 门控` | P0、L1、W2 证据与 H1 模型包 | P0/L1 已验收；W2 未完成 | 审批后由 5090 验证 H1 |
...
- 核心执行步骤：P0 工件验证、L1 静态调用链、W2 Attempt 1、Correction 01/02、H1 离线模型包。
...
- 工程风险：H1 尚未由 5090 独立核验、离线加载，GMTP detection-core 未完成。
...
- P0：在新的人工审批门后，让 5090 独立验证 H1、离线加载固定 revision，并继续 W2 detection-core 最小 smoke。

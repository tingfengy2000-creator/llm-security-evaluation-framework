...
## 旧值映射：“没读到”不等于“历史不存在”（2026-08-28）

人工 CSV 常会在技术列名后追加中文说明或换行。若生成器直接用 `row.get(canonical_name, "[V1_ABSENT]")`，列名稍有变化就会
把存在的历史答案伪装成缺失。本次 B Phase1 的 108 个任务就属于这种问题，B Phase2 也有两个字段受影响。

可复用规则是：只允许已审计的精确 alias 映射；映射后冲突必须停止；每个 phase 显式枚举哪些字段历史上真正不存在；任何其他缺失 fail closed。测试不能只查“文件能生成”，还必须检查每个字段的旧值分布和合法 absent 集合。

## 候选自包含性：标注人能猜出不等于样本合格（2026-08-28）

...
| 当前任务 | `PILOT4-R3-FINAL-RAW-LOCK-COMPARISON-AND-PROTOCOL-ACCEPTANCE-EVIDENCE-V2-01`：锁定 37 行 R3 raw、受控比较 Expected V2、评估冻结门并形成 Acceptance Evidence V2。 |
| 当前审批门 | Owner 审查 `RECOMMEND_TARGETED_REPAIR / EXPECTED_V2_SYSTEMIC_REPAIR_BLOCKER`，决定是否批准 additive Expected V3 的 7-field/6-candidate correction；protocol acceptance 与 A/B 仍为独立后续 gate。 |
...
| 当前主要阻塞项 | R3 冻结门 A/B/C 失败：M2 `3/16`、同根 overall 最大簇 `3`、controls overall `13/16`；审计指向 Expected V2 的 7 个字段/6 个候选，formal protocol、Scale Pilot、Dataset 和 method effectiveness 均未冻结。 |
| 当前允许宣称 | R3 raw `12062 bytes` / SHA `80a10a1e...0b4441` 不可变；37/37 QA、raw-lock-before-Expected、M4/BR18 与 M8 均 PASS；targeted agreement 与 9-field taxonomy 已完成；只能建议 additive Expected repair，不能称协议接受。 |
...
| Stage 6.1 | Hidden Knowledge Poisoning Detection | 隐蔽污染检测 | LR1/R0/FU1 accepted；P1-R1 framework accepted；Pilot4 targeted R3 completed；Expected V2 repair blocker remains；formal work not started | `PILOT_IN_PROGRESS / NOT_FORMAL_EXPERIMENT` | [长期需求](long_term_research_requirements.md) | Owner Expected V3 decision, then separate protocol acceptance |
...
| GATE-S6.1-P1-PILOT4-PROTOCOL-ACCEPTANCE | Final blind comparison and protocol acceptance | first/final raw locks；lock-before-expected proof；Phase1/Phase2 metrics；43 mismatch taxonomy；acceptance evidence | `RECOMMEND_TARGETED_REPAIR / OWNER_PROTOCOL_ACCEPTANCE_PENDING / NO_AB_DISTRIBUTION` | Owner may approve additive guide/expected/evidence repair；after targeted validation Owner separately accepts or rejects protocol | auto acceptance、A/B、GT、240-group、Dataset freeze、Detector/Training、5090、Formal Experiment | 项目负责人 / PODR-083 |

...
| 当前任务 | `S6.1-R0-FU1-P0 = COMPLETED_PENDING_OWNER_REVIEW`；parent FU1 `APPROVED / LOCAL-FIRST / WORKER-GATED`。 |
| 当前审批门 | owner reviews P0 and separately decides W1/W2；`FORMAL_EXPERIMENT = NOT STARTED`。 |
| 下一批准任务 | optional `FU1-W1` and/or `FU1-W2` only；both remain `NOT APPROVED`；S6.1-P1 remains closed。 |
...
| 当前主要阻塞项 | `BLK-S6.1-FU1-W1-001`、`BLK-S6.1-FU1-W2-001` and formal-environment items；redistribution license remains separate。 |
...
| S6.1-R0-FU1-P0 | Targeted External Baseline Feasibility Resolution | dataset/attack/source/call-path and Worker contract freeze | completed pending owner review | `CONTROL_PLANE_CONTRACT_CANDIDATE` | [FU1 resolution](../research/stage6_1_hidden_knowledge_poisoning/s6_1_r0_fu1_targeted_resolution.md)、PODR-051 | owner separately decides W1/W2 |
| Stage 6.1 | Hidden Knowledge Poisoning Detection | 隐蔽污染检测 | LR1/R0 accepted；FU1-P0 pending owner review；P1 not started | `PLANNED` | [长期需求](long_term_research_requirements.md) | required FU1 evidence then separate P1 decision |
...
| BLK-S6.1-FU1-W1-001 | 2026-07-31 | Stage 6.1 | FU1-W1 | RTX5090 | high | exact released-artifact assembly manifest has not run on Worker | P1 lacks accepted runtime evidence for attack input identity | W1 engineering validation | P0 exact sparse-checkout contract | source analysis only | contract frozen | run W1 under separate approval | not approved | do not regenerate or invoke API | NOT_RECORDED | N/A | N/A | NOT_RECORDED | OPEN |
| BLK-S6.1-FU1-W2-001 | 2026-07-31 | Stage 6.1 | FU1-W2 | RTX5090 | high | exact GMTP detector core/models/scores are unvalidated on modern Worker environment | P1 lacks executable detector baseline evidence | W2 engineering validation | P0 exact model/sample/call contract | source analysis only | contract frozen | run W2 under separate approval | not approved | no Torch downgrade/algorithm rewrite | NOT_RECORDED | N/A | N/A | NOT_RECORDED | OPEN |
...
| BLK-S6.1-LR1-001 | 2026-07-31 | strict comparison 准入 | high | Paper 1 strict comparison | `OPEN` | [Artifact Registry](../research/stage6_1_hidden_knowledge_poisoning/external_artifact_registry.md)、accepted R0/FU1-P0 | source identity planning improved；internal research, strict comparison and redistribution remain separate | accepted W1/W2 plus later P1/formal protocol/environment | await P0 owner review |
...
| BLK-S6.1-P1-001 | 2026-07-31 | P1 source planning | high | PoisonedRAG comparison identity | `RESOLVED` | NQ/released-artifact/partial-equivalence contract frozen | no runtime claim | `RESOLVED_BY_P0_CONTRACT_FREEZE` | preserve W1 gate |
...
| BLK-S6.1-FU1-W1-001 | 2026-07-31 | Worker validation | high | PoisonedRAG input identity | `OPEN` | exact W1 candidate contract | no execution workaround | accepted W1 evidence | await owner approval |
| BLK-S6.1-FU1-W2-001 | 2026-07-31 | Worker validation | high | GMTP executable core | `OPEN` | exact W2 candidate contract | no downgrade/reimplementation | accepted W2 evidence | await owner approval |
...
| GATE-S6.1-R0-FU1-P0 | Targeted baseline source/contract freeze | NQ/attack identity、official BEIR、GMTP detector core、SafeRAG artifacts、W1/W2 candidates | `COMPLETED_PENDING_OWNER_REVIEW` | owner may separately approve W1 and/or W2 | automatic Worker execution、S6.1-P1、formal experiment | 项目负责人 |
...
`S6.1-R0 = HUMAN_ACCEPTED_WITH_BLOCKERS`。R0-FU1 is `APPROVED / LOCAL-FIRST / WORKER-GATED` and P0 is
`COMPLETED_PENDING_OWNER_REVIEW`；W1/W2、S6.1-P1、Dataset、Detector、training and formal RAG experiment remain
unapproved/not started。

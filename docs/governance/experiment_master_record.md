...
| 当前任务 | `GOV-P1-MANDATORY-DOCUMENTATION-CLOSEOUT-01 = PAPER1_PERSISTENT_DOCUMENTATION_CLOSEOUT_GOVERNANCE`；不改变当前实验门。 |
| 当前审批门 | Owner 在全新隔离 reviewer context 只提供 `PILOT4_EXTERNAL_BLIND_PHASE1_PACKET.md` 与 `PILOT4_EXTERNAL_BLIND_PHASE1_GUIDE.md`；Phase1 return 未完成 schema、72/72、hash-lock 与 immutable 验证前 Phase2 withheld。 |
...
| 当前主要阻塞项 | `EXTERNAL_PHASE1_RETURN_PENDING`；Phase2 withheld；formal protocol numeric parameters、Scale Pilot、Dataset 和 method effectiveness 均未冻结。 |
| 当前允许宣称 | Phase1/Phase2 已结构隔离；Phase1 candidate-only visibility QA passed；Phase2 prebuilt and withheld；不代表外部语义复核、独立人工接受、Ground Truth 或 Detector effectiveness。 |
...
| Stage 6.1 | Hidden Knowledge Poisoning Detection | 隐蔽污染检测 | LR1/R0/FU1 accepted；P1-R1 framework accepted；Pilot4 Phase1 packet ready and Phase2 withheld；formal work not started | `PILOT_IN_PROGRESS / NOT_FORMAL_EXPERIMENT` | [长期需求](long_term_research_requirements.md) | Owner distributes exactly Phase1 packet + guide to a fresh isolated reviewer |

...
- s6_t5_6_p1_h1_status: `completed_pending_human_acceptance`；只加固顺序解析、精确 UID 重复、预算 cutoff 与 Context trace identity；不创建业务代码。
- s6_t5_6_p1_h2_status: `completed_pending_human_review`；只关闭活动规格顺序、Trace decision partition 与 Package identity 协议矛盾；不创建业务代码。
...
- next_stage: 先人工审查 `S6-T5.6-P1-H2`，再验收 P1/H1；父任务 `S6-T5.6` 和 `S6-T5.7+` 仍为 `NOT APPROVED`，不得自动开始。
...
- Codex 恢复入口：从仓库根 `AGENTS.md` 开始，并读取 `docs/governance/current_work_state.md`；GOV-PODR1、S6-T5.2、S6-T5.3-P1、S6-T5.3-H1、S6-T5.3 DenseRetriever、S6-T5.4、S6-T5.5-P1 与 P1-H1 均已获人工验收；
...
- 当前协议：`S6-T5.6-P1` 冻结临时 Binding 的精确 renderer 预算、stable prefix selection、safe build trace、Package identity 和 structural abstention；H1 将正文解析改为 sequential resolution；H2 统一活动文档的顺序，冻结 instruction-budget decision partition 和从 `build_trace.trace_hash` 派生的 Package identity；P1/H1 待人工验收、H2 待人工复核，均未创建 ContextBuilder；

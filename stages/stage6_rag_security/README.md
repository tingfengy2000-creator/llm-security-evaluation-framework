...
- 当前协议：`S6-T5.6-P1` 冻结临时 Binding 的精确 renderer 预算、stable prefix selection、safe build trace、Package identity 和 structural abstention；H1 将正文解析改为 sequential resolution；H2 统一活动文档的顺序，冻结 instruction-budget decision partition 和从 `build_trace.trace_hash` 派生的 Package identity；P1/P1-H1/P1-H2 已通过人工验收，但均未创建 ContextBuilder；

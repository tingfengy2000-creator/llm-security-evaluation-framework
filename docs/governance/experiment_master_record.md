...
| 当前任务 | `PILOT4-PHASE2-FINAL-RETURN-LOCK-EXPECTED-COMPARISON-AND-PROTOCOL-ACCEPTANCE-01`：锁定 final Phase2 raw、受控解锁 mapping/expected、分类分歧并生成 protocol recommendation。 |
| 当前审批门 | Owner 审查 `RECOMMEND_TARGETED_REPAIR`：冻结 16 条 primary-status 语义边界并决定 expected/minimum-evidence 与 `BR-18F1D39495` 的追加式修复；protocol acceptance 与 A/B 仍为独立后续 gate。 |
...
| 当前主要阻塞项 | `OVERALL_FACT_STATUS_CURRENT_VS_LEGITIMATE_BOUNDARY_16_ROWS`、targeted expected/minimum-evidence repair 与 `BR-18F1D39495_EVIDENCE_POOL_DESIGN_DEFECT`；formal protocol、Scale Pilot、Dataset 和 method effectiveness 均未冻结。 |
| 当前允许宣称 | First/final Phase2 raw 分别不可变；同一 reviewer retry 后 `SOURCE_UNREACHABLE 23→0`；raw-lock-before-expected PASS；Phase1/Phase2 expected comparison 与 43-field taxonomy 已完成；只能建议 targeted repair，不能称协议接受。 |
...
| Stage 6.1 | Hidden Knowledge Poisoning Detection | 隐蔽污染检测 | LR1/R0/FU1 accepted；P1-R1 framework accepted；Pilot4 final blind comparison completed with targeted-repair recommendation；formal work not started | `PILOT_IN_PROGRESS / NOT_FORMAL_EXPERIMENT` | [长期需求](long_term_research_requirements.md) | Owner protocol repair/acceptance decision |

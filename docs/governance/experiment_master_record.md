...
| 当前任务 | `PILOT4-EXPECTED-V3-TARGETED-CORRECTION-AND-FROZEN-GATE-RECOMPUTE-01`：以 reviewer-blind Evidence/Guide 审计形成 additive Expected V3，并用锁定 R3 raw 重算冻结门。 |
| 当前审批门 | A–F 已全部 PASS；Owner 审查 `RECOMMEND_ACCEPT_WITH_NONBLOCKING_NOTES` 并明确接受 Protocol 或退回修复。若接受，A/B execution 仍是独立后续 gate。 |
...
| 当前主要阻塞项 | 无校准/systemic blocker；唯一开放 gate 是 `OWNER_PROTOCOL_ACCEPTANCE_PENDING`。Formal A/B、Scale Pilot、Dataset 和 method effectiveness 均未冻结。 |
| 当前允许宣称 | Expected V3 精确更正 7 字段/6 候选且先于 reviewer load 锁定；M2 `2/16`、controls overall/exact `16/16`、all exact `35/37`、A–F PASS；仅建议接受，不得称 Owner 已验收。 |
...
| GATE-S6.1-P1-PILOT4-PROTOCOL-ACCEPTANCE | Final blind comparison and protocol acceptance | first/final raw locks；targeted R3；reviewer-blind Expected V3；frozen gates A–F；Acceptance Evidence V3 | `FROZEN_GATES_A_F_PASS / RECOMMEND_ACCEPT_WITH_NONBLOCKING_NOTES / OWNER_PROTOCOL_ACCEPTANCE_PENDING / NO_AB_DISTRIBUTION` | Owner explicitly accepts protocol or returns for repair；if accepted, Owner later separately approves A/B execution | auto acceptance、R4、A/B、GT、240-group、Dataset freeze、Detector/Training、5090、Formal Experiment | 项目负责人 / PODR-086 |

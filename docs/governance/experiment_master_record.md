...
| 当前任务 | `PILOT4-EXTERNAL-BLIND-PHASE1-RETURN-LOCK-AND-DEFECT-TRIAGE-01`：锁定真实外部 Phase1 return，验证结构并准备盲态缺陷处置门。 |
| 当前审批门 | Owner 只审查五条 blind-ID candidate defect triage 并追加处置决定；mapping、expected contract 和 Phase2 保持关闭。 |
...
| 当前主要阻塞项 | `PHASE1_CANDIDATE_DEFECT_TRIAGE_PENDING`；前五项 return-lock 事实通过，第六项 triage fact 为 false；formal protocol numeric parameters、Scale Pilot、Dataset 和 method effectiveness 均未冻结。 |
| 当前允许宣称 | External Phase1 raw return 真实收到并 immutable lock；five-column/schema/72-ID/enum/reason QA passed；descriptive Phase1 counts recorded；不代表 expected-label accuracy、独立人工接受、Ground Truth 或 Detector effectiveness。 |
...
| Stage 6.1 | Hidden Knowledge Poisoning Detection | 隐蔽污染检测 | LR1/R0/FU1 accepted；P1-R1 framework accepted；Pilot4 Phase1 return locked, Owner defect triage pending and Phase2 withheld；formal work not started | `PILOT_IN_PROGRESS / NOT_FORMAL_EXPERIMENT` | [长期需求](long_term_research_requirements.md) | Owner blind-only disposition of five candidate-issue rows |

...
| 当前任务 | `PILOT4-OWNER-PROTOCOL-ACCEPTANCE-AND-AB-EXECUTION-PREFLIGHT-01`：登记 Owner 正式验收，冻结 Accepted Stack V1，关闭校准并准备 A/B execution approval packet。 |
| 当前审批门 | Pilot4 Protocol 已由 Owner 以 `ACCEPTED_WITH_NONBLOCKING_NOTES` 接受；A/B execution 仍等待 Owner 单独批准、roster 与隔离声明。 |
| 下一批准任务 | Owner 审查 A/B execution packet；A/B 72 annotation、240-group、Dataset、Formal Detector、Training 和 Formal Experiment 均未自动启动。 |
...
| Stage 6.1 | Hidden Knowledge Poisoning Detection | 隐蔽污染检测 | LR1/R0/FU1 accepted；P1-R1 framework accepted；Pilot4 calibration gates pass；Owner protocol decision pending；formal work not started | `PILOT_IN_PROGRESS / NOT_FORMAL_EXPERIMENT` | [长期需求](long_term_research_requirements.md) | Owner protocol acceptance decision, then separate A/B execution approval |
...
| GATE-S6.1-P1-PILOT4-A-B-EXECUTION | Formal Pilot4 A/B annotation execution | two independent annotators；separate randomized opaque IDs；Phase1 dual lock before Phase2；four-return lock before mapping/Expected；agreement and Owner disagreement-only adjudication | `EXECUTION_APPROVED / BOTH_PHASE1_PACKETS_READY / WAITING_FOR_OWNER_DISTRIBUTION / PHASE2_WITHHELD` | Owner manually sends only each annotator's five Phase1 files and returns both raw CSVs for immutable lock | early Phase2、mapping/Expected/agreement before four locks、GT、240-group、Dataset/Detector/Training/formal experiment | 项目负责人 / PODR-088 |
...
calibration stops、Owner protocol acceptance is complete and A/B execution is separately approved；both Phase1 packets are ready
but actual distribution is not yet recorded，while Phase2 remains withheld。A/B annotation completion、240-group、Dataset、

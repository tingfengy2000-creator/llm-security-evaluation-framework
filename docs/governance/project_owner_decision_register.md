...
# PODR-104 — Final72 V1.1 first LOGO Detector execution approval (2026-09-22)

Owner separately approved the first actual non-Oracle 21-feature Final72 development-only Document Poison Detector fit after V1.1 Phase A passed. The approved execution freezes 24 whole-group LOGO folds, nine prespecified S/E/P/T or full-minus-view L2 Logistic Regression models, fold-local preprocessing, fixed 0.5 diagnostic threshold, 2000 matched-group bootstrap draws and at least 200 within-group pseudo-poison permutations. No feature/GT/retriever/Registry/query changes, tuning, calibration, Stage B, formal benchmark or superiority claim is authorized. Execution produced the hash-locked run `paper1_final72_document_detector_v1_1_logo_20260922`; observed `MULTIVIEW_SIGNAL_MIXED / PROTOTYPE_VALID_WITH_LIMITATIONS` is an engineering finding, **not** an Owner acceptance of formal effectiveness. Next analysis requires separate Owner approval.

# PODR-105 — Frozen-result failure analysis approval, evidence-gap decision pending (2026-09-22)

Owner approved read-only Final72 V1.1 detector failure diagnosis and formal-scale hypothesis refinement, using frozen feature values, locked OOF results and post-lock labels. The approval explicitly forbids re-fitting, hyperparameter/threshold tuning, new classifiers and any mutation of GT/Feature Set/Matrix/Registry/query/previous detector results. Input preflight found only Full SEPT sample-level OOF persisted; eight view/ablation model score vectors are absent from the indexed run. This is `P1-FDD-FAILURE-ANALYSIS-EVIDENCE-BLOCKER-01 / HUMAN_DECISION_REQUIRED / Auto Continue=NO`, **not** an Owner decision to rerun or waive required score-shift analyses. The pending A/B/C choice is documented in the [decision-ready blocker](../research/stage6_1_hidden_knowledge_poisoning/method_engineering/PAPER1_DOCUMENT_DETECTOR_FAILURE_ANALYSIS_EVIDENCE_BLOCKER_01.md).

# PODR-106 — Owner resolution of omitted-OOF blocker (2026-09-22)

Owner superseded PODR-105's pending choice with `A_FIRST_THEN_B_IF_NEEDED`: bounded read-only original-artifact search, then one-time exact **evidence reconstruction** only if authentic eight-model OOF was not verifiable. Option C was not selected. Original Full OOF and all historical summaries remain canonical and immutable. Reconstruction must match exact frozen GT/Feature Set/Matrix/code/environment/folds/config/preprocessing; original Full sample-level scores and all nine machine summaries must pass strict predeclared numerical parity before failure analysis resumes. Any failure would stop at `REPRODUCIBILITY_BLOCKER`, without switching to C. Owner additionally requires all future single-view/ablation/baseline/formal runs to retain per-sample scores, fold identity, model ID and input hashes. This authorizes no tuning, formal benchmark execution, risk calibration, Stage B model or Paper 1 result claim.

# PODR-107 — Formal240 annotation V4 and construction protocol freeze (2026-09-22)

Owner closed the Final72 development-construction phase and approved `P1-FORMAL240-ANNOTATION-SCHEMA-V4-AND-CONSTRUCTION-PROTOCOL-FREEZE-01` for additive **protocol, schema and 240 empty group-slot matrix only**. The target is five domains × four HKP × target S1/S2/S3 × four independent chains, eventually 720 C/P/H candidates. Formal V4 restricts `version_claim_status` to document-level version metadata, separately judges substantive content and authority, and adds `ZERO_EXTERNAL_EVIDENCE_REQUIRED` for local factual contradiction; derived S never comes from target S. This future-formal decision supersedes the older Pilot revision-content interpretation **only for Formal240**. Pilot4 Guide V3.2, raw A/B, Owner adjudication, Final72 GT and historical minimum values remain immutable. The group-aware split algorithm and seed `20260922` are frozen without execution. No Formal candidates, annotations, GT, dataset freeze, Detector fit or paper result are approved by this task. Next gate: separate Owner approval for D1 enterprise-HR Wave. [Owner protocol review packet](../research/stage6_1_hidden_knowledge_poisoning/formal240/PAPER1_FORMAL_240_GROUP_PROTOCOL_OWNER_ACCEPTANCE_PACKET.md).

# PODR-108 — Formal240 D1 Canary and pre-annotation QA approval (2026-09-23)

Owner separately approved D1 enterprise-HR construction, but only an eight-slot/24-candidate Evidence-first Canary in this task. Human annotation rework minimization and an auditable pre-annotation quality gate are hard requirements. Two independent external GPT sessions may conduct candidate-only Phase1 and frozen-Evidence Phase2 QA, without labels/mapping/expected answers; their outputs are construction checks, not Ground Truth. A failed fact/evidence/schema/style gate requires additive repair and fresh blind review. Canary Owner acceptance is a separate decision. No remaining 40 D1 groups, Human A/B release, split, Detector training or formal paper claim is authorized automatically. `OR-064 / REL-2026-0073` records the current control-plane state; the historical `PODR-107` D1-not-started sentence is superseded only for this bounded Canary.

# PODR-109 — Formal240 D1 Canary independent Phase1 raw-lock directive (2026-09-23)

Owner supplied attachment 1 as R1 and attachment 2 as R2, attested two independent external GPT reviewers used Phase1 package V2 and schema V2, and stated Phase2 had not been provided. Owner authorized only byte-preserving raw locks, exact 24-row/schema/ID/order/enum validation and R1/R2 categorical agreement. If all pass, Phase2 release becomes authorized; this does not itself distribute or analyze Phase2. C/P/H, HKP, S, Expected, GT and Owner mapping are forbidden in reviewer evaluation. The raw copies pass and are indexed in [Phase1 lock record](../research/stage6_1_hidden_knowledge_poisoning/formal240/PAPER1_FORMAL_D1_CANARY_BLIND_PHASE1_RAW_LOCK_RECORD_V1.md). This decision does not accept the Canary or release Human A/B; `OR-065 / REL-2026-0074`.

# PODR-110 — D1 Canary Phase2 process invalidation and forward V4.1 clarification (2026-09-23)

Owner supplied the current R1/R2 Phase2 composite return and identified the purported R1's explicit note that its session's Phase1 role was R2. This invalidates R1 cross-phase reviewer independence and prevents using the current pair for independent Canary agreement. Preserve both raw values byte-for-byte; classify R1 as non-independent auxiliary QA and keep reviewer/session/routing incidents at review-run level, not candidate `phase2_issue`. Owner approved a forward-only Formal V4.1 clarification: publication/adoption/order/date alone is not automatically a version claim; substantive operational/evidentiary actors are not automatically authority claims; reviewer-run defects are not row-level issues. Enums, Formal V4, Schema V4, Candidate and Evidence remain unchanged. The current pair may inform *diagnostic* schema analysis only. Owner requires two fresh isolated R3/R4 sessions, each Phase1 raw lock before Phase2 release, then Phase2 raw lock and Owner Canary decision. No Human A/B, remaining40, split or training. See [incident](../research/stage6_1_hidden_knowledge_poisoning/formal240/PAPER1_FORMAL_D1_BLIND_REVIEW_PROCESS_INCIDENT_V1.md), `OR-066 / REL-2026-0075`. This is an additive correction to the prior Phase1 continuity expectation, not a rewrite of that historical lock.

# PODR-111 — D1 Canary fresh reviewer identity and isolation decision (2026-09-23)

Owner superseded **only the future R3/R4 reviewer names and execution gate** with `R3-gpt` and `R4-codex`. R3-gpt needs a new independent GPT session; R4-codex needs a new projectless isolated Codex session, not a fork/continuation of the current construction/control-plane context and without access to the project repository, handoff folder, old reviews, Owner packet, mapping, labels or earlier diagnostics. The two receive semantically equivalent phase-appropriate materials. Each run requires separate review-run attestation of identity, allowed-file exposure and Phase1-before-Phase2 ordering; these facts never enter candidate JSON. Any R4 repository/context/hidden-answer exposure makes its result auxiliary QA only. Both fresh Phase1 raw locks, 24/24 validation and attestations must pass before either Phase2 release. Different reviewer systems are **not** a model benchmark. [V2 plan](../research/stage6_1_hidden_knowledge_poisoning/formal240/PAPER1_FORMAL_D1_CANARY_R3_GPT_R4_CODEX_REVIEW_PLAN_V2.md), `OR-067 / REL-2026-0076`. Old R1/R2 identities and raw, Formal V4/V4.1, Schema V4, Candidate, Evidence and 240 matrix remain unchanged; no fresh review, Canary acceptance, Human A/B, remaining40, split or training is approved as completed by this decision.

# PODR-112 — D1 Canary R3/R4 Phase1 input correction and two-row naturalness repair (2026-09-23)

Owner corrected the mistaken earlier paste interpretation: `R3-gpt` did **not** return premature Phase2, so no R3 process incident should be created. Owner explicitly assigned the newly supplied attachment 1 to `R4-codex` and attachment 2 to `R3-gpt`. Owner confirmed both Phase1 sessions were fresh and isolated, R4 was a new projectless task in an empty directory, each saw only the Phase1 V2 packet/schema/matching prompt, neither used external search or accessed repo/old returns/Owner packet/mapping/labels/Phase2, and explicitly authorized `OWNER_ATTESTED` rather than claiming system-verifiable isolation. The two raw files must remain byte-immutable. If their independent categorical checks confirm the two common “第四条下” `MINOR_ISSUE` findings, Owner authorizes only the fixed-span text substitution in those two candidates, with V3/new opaque IDs, unchanged evidence and additive repair ledger. The original reviewer sessions must each recheck only the two repaired candidates. Phase2 V4 may be prebuilt but must stay withheld until both targeted Phase1 raw checks pass and V3 is accepted. No Human A/B, remaining40, split, training or Canary acceptance is authorized. [Execution record](../research/stage6_1_hidden_knowledge_poisoning/formal240/PAPER1_FORMAL_D1_CANARY_R3_R4_PHASE1_LOCK_AND_TARGETED_REPAIR_RECORD_V1.md); `OR-068 / REL-2026-0077`.

# PODR-113 — D1 Canary primary Phase1 targeted closeout and optional R5 role (2026-09-26)

Owner designated `R3-gpt` and `R4-codex` as the sole primary gating pair; `R5-claude` is optional, non-gating auxiliary QA and never a GT voter or mandatory tie-breaker. The Owner supplied two R3/R4 targeted Phase1 JSON arrays within the current composite attachment, instructed immutable byte locks, exact 2/2 validation, 22+2 overlay and conditional Candidate V3/Phase2 V4 release after mechanical re-QA. R5's second response is a **targeted Phase1 repair re-review**, not Evidence Phase2. An R5 naturalness disagreement based only on the designed internal contradiction does not itself justify modifying that contradiction or delaying the primary gate. R5 absence/quota cannot block Wave progression; a newly substantiated candidate/evidence defect would still be escalated. Phase2 release is authorization for Owner/coordinator distribution to the same original R3/R4 sessions, not execution or Canary/Human A/B acceptance. Owner later supplied two actual R5 files, byte-locked and independently validated as 24/2 auxiliary Phase1 records; this does not change reviewer roles. [Primary closeout](../research/stage6_1_hidden_knowledge_poisoning/formal240/PAPER1_FORMAL_D1_CANARY_PHASE1_FINAL_CLOSEOUT_V1.md); `OR-069 / REL-2026-0078`.

# PODR-114 — Formal240 V4.2 candidate-only version scope and D1 targeted confirmation (2026-09-26)

Owner approved a **global prospective semantic clarification**, not a choice of R3 over R4 or a majority vote. A bare named-law/article substantive rule without explicit candidate version semantics is `version_claim_status=NOT_PRESENT`; official Evidence metadata cannot reverse-create a candidate claim. Explicit amendment/revision wording, including an amendment-decision title, makes the claim present, with correctness checked against frozen Evidence. A supported explicit original/historical-versus-revised comparison is `overall_fact_status=LEGITIMATE_VERSION_OR_HISTORY`. Apply these rules mechanically to the 15 D1 Phase2 disagreement IDs as an additive Owner-semantic overlay, retaining original R3/R4 raw, V4/V4.1, Candidate and E1/E2. Send the same blind 15-row package and V4.2 clarification to the original isolated R3-gpt/R4-codex sessions for **targeted rule-confirmation**, not fresh independent annotation; R5 is optional. No Owner overlay value or prior reviewer answer may appear in that package. Require both target fields to agree 15/15 and resolve any new blocking defect before a separate Canary acceptance decision. Human A/B, remaining40, split and training remain unauthorized. Original Owner directive bytes/SHA256 are preserved privately (`e6553e08cf05567ed1ab9598b5f18c3a08dfb17e50ca8e4954e10cc6a6554f39`); [V4.2 guide](../research/stage6_1_hidden_knowledge_poisoning/formal240/PAPER1_FORMAL_ANNOTATION_GUIDE_V4_2_VERSION_SCOPE_DECISION.md) and [preparation report](../research/stage6_1_hidden_knowledge_poisoning/formal240/PAPER1_FORMAL_D1_CANARY_VERSION_SCOPE_REFINEMENT_REPORT_V1.md); `OR-070 / REL-2026-0080`.

# PODR-115 — Formal240 D1 Canary conditional final acceptance and remaining40 boundary (2026-09-26)

Owner explicitly directed that the 8-group/24-candidate D1 Canary be accepted **only if all enumerated scientific, Evidence, construction, five-view, independence, surface and retrieval-execution hard gates pass**; a local blocker instead requires targeted repair, not a blanket restart. [Run02 bounded audit](../research/stage6_1_hidden_knowledge_poisoning/formal240/PAPER1_FORMAL_D1_CANARY_FINAL_OWNER_ACCEPTANCE_AUDIT_V1.md) records all gates PASS and the private matrix SHA256 `9847bdc5ae4b053846007a496ca779ef35a47e99a6e1db2c073e86b65afb07ee`. Thus the Owner's conditional decision sets `D1_CANARY_ACCEPTED_FOR_FULL_WAVE_CONSTRUCTION=TRUE`. The preliminary Run01 metadata table is preserved but non-authoritative because it overidentified a site operator as page publisher; Run02 separates host, publisher, issuer and source attribution. Acceptance permits **only** a future, separately Owner-approved remaining40 construction task; it does not itself execute that task, release Human A/B, create GT, split data, train a detector or establish a formal paper result. R3/R4 isolation is Owner-attested, and R5 remains optional/non-gating. `OR-071 / REL-2026-0082`.

# PODR-116 — Formal240 D1 remaining40 sequential construction approval (2026-09-26)

Owner now explicitly approves `P1-FORMAL240-D1-REMAINING40-CONSTRUCTION-01`: four ordered HKP1→HKP4 batches of ten frozen matrix groups/thirty C/P/H candidates each. A batch may lead automatically to the next only after **all** evidence, construction, view, query, R3/R4 independent Phase1 and Phase2 hard gates pass; any hard blocker stops. External reviewers cannot be impersonated locally; pauses at their return gates do not require a new scientific approval. R5 is optional auxiliary only. No Human A/B, GT, formal split, Detector training/result, or D2–5 construction is approved. Batch-1's [preblind record](../research/stage6_1_hidden_knowledge_poisoning/formal240/PAPER1_FORMAL_D1_HKP1_BATCH1_PREBLIND_STATUS_V1.md) is an execution snapshot, **not** Owner acceptance of its candidates. `REL-2026-0084`.

# PODR-117 — D1 HKP1 Batch-1 R4 one-run environment equivalence exception (2026-09-26)

Owner explicitly approved **only the fresh D1 HKP1 Batch-1 R4-codex Run02 Phase1** as a bounded equivalent to the otherwise frozen projectless reviewer mode, and allowed Phase2 release to that **same isolated R4 session** after the remaining Phase1 hard gates pass. This is not a retroactive assertion that R4 was projectless. Owner states the new Project had no inherited files/instructions and saw only its own V4 Phase1 package/schema/prompt; read-only inspection of the Owner-specified Project directory found only the matching 7,907-byte R4 raw JSON, no hidden instruction file or `.git`. That verifies the directory's present state, not platform-level session history or inherited settings; isolation remains `OWNER_ATTESTED_NOT_MACHINE_VERIFIED`. The prior first-run R3 routing incident stays invalid as a primary review, and all old raw is preserved. `PODR-111` continues to govern every **future batch**: R3 new independent GPT conversation and R4 new `projectless` empty-directory task at Phase1; same accepted sessions continue into that batch's Phase2. No general new-Project permission, Candidate/Guide/Evidence change, Batch-1 acceptance, HKP2, Human A/B, GT, split or training is granted. [Bounded gate](../research/stage6_1_hidden_knowledge_poisoning/formal240/PAPER1_FORMAL_D1_HKP1_BATCH1_FRESH_PHASE1_LOCK_AND_R4_ENVIRONMENT_GATE_V2.md), [Phase2 handoff](../research/stage6_1_hidden_knowledge_poisoning/formal240/PAPER1_FORMAL_D1_HKP1_BATCH1_PHASE2_RELEASE_RECORD_V1.md), `OR-073 / REL-2026-0085`.

# PODR-118 — D1 HKP1 Batch-1 V4.3 semantic closeout and remaining90 MegaWave (2026-09-27)

Owner provided R3/R4 V4 Phase2 raw, ordered immutable locks and independent QA, and approved no further external six-item rereview. A historical/temporal substantive comparison alone does not assert a **document** version; only candidate-visible explicit document-version semantics make `version_claim_status=PRESENT_*`. This is an additive V4.3 rule and per-case Owner overlay, not R3 precedence, raw editing or GT. If all Phase1/Phase2/construction hard gates pass, accept HKP1 Batch-1 only for D1 continuation. Owner supersedes the earlier per-HKP external pause: build HKP2→HKP3→HKP4 internally with separate Evidence-first hard gates, then one cross-group-shuffled 90-candidate R3/R4 MegaWave, fresh isolated sessions, Phase1 followed by same-session Phase2. Only observed platform limits permit a 60+30 fallback; R5 remains optional. The present R4 new-Project exception is not transferable: the MegaWave requires a fresh projectless empty-directory R4 task. Hard blockers stop; Human A/B, GT, split, detector training and formal results remain unauthorized. [Batch-1 acceptance](../research/stage6_1_hidden_knowledge_poisoning/formal240/PAPER1_FORMAL_D1_HKP1_BATCH1_PHASE2_ACCEPTANCE_RECORD_V1.md); [prospective routing](../research/stage6_1_hidden_knowledge_poisoning/formal240/PAPER1_FORMAL_D1_REMAINING90_MEGAWAVE_ROUTING_V1.md). `OR-074 / REL-2026-0086`.

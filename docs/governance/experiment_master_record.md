...
| 当前任务 | `PILOT4-A-B-DUAL-PHASE1-RAW-LOCK-AND-HUMAN-FRIENDLY-PHASE2-RELEASE-01`：锁定 A/B Phase1 raw，关闭双门并准备 Phase2 V3 人类工作簿。 |
| 当前审批门 | Pilot4 Protocol/A-B execution 已批准；A/B Phase1 raw 已分别锁定且双门 PASS；Phase2 release allowed，A/B Phase2 V3 均 ready，但尚未分发。 |
| 下一批准任务 | Owner 分别向 HUMAN-A01/HUMAN-B01 发放各自三个 Phase2 V3 文件；mapping/Expected/agreement/GT 和所有下游实验仍关闭。 |
...
| 当前主要阻塞项 | 无 Phase2 分发 blocker；开放 gate 是 `WAITING_FOR_OWNER_DUAL_PHASE2_DISTRIBUTION`。agreement、Ground Truth、Scale Pilot、Dataset 和 method effectiveness 均未建立。 |
...
| Stage 6.1 | Hidden Knowledge Poisoning Detection | 隐蔽污染检测 | LR1/R0/FU1 accepted；P1-R1 framework accepted；Pilot4 protocol/A-B execution approved；dual Phase1 locked；Phase2 V3 ready；formal work not started | `PILOT_IN_PROGRESS / WAITING_OWNER_DUAL_PHASE2_DISTRIBUTION / NOT_FORMAL_EXPERIMENT` | [长期需求](long_term_research_requirements.md) | Owner separately distributes each annotator's three Phase2 V3 files；no mapping/Expected/agreement until both returns lock |

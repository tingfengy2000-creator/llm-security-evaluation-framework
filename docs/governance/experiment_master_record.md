...
| 当前任务 | `S6.1-R0-FU1-L1 = HUMAN_ACCEPTED`；P0 also `HUMAN_ACCEPTED`；parent FU1 `APPROVED / LOCAL-FIRST / WORKER-GATED`。 |
| 当前审批门 | owner decides whether to approve the frozen W2 contract；`FORMAL_EXPERIMENT = NOT STARTED`。 |
| 下一批准任务 | optional `FU1-W2` only；`READY_FOR_OWNER_EXECUTION_APPROVAL / NOT_YET_EXECUTED`；S6.1-P1 remains closed。 |
...
| 最近工程验证 | L1 exact released-artifact/schema/deterministic-assembly validation；W2 contract hardening without execution。 |
...
| S6.1-R0-FU1-P0 | Targeted External Baseline Feasibility Resolution | dataset/attack/source/call-path and Worker contract freeze | human accepted | `HUMAN_ACCEPTED_CONTROL_PLANE_CONTRACT` | [FU1 resolution](../research/stage6_1_hidden_knowledge_poisoning/s6_1_r0_fu1_targeted_resolution.md)、PODR-051/052 | W2 owner decision |
...
| Stage 6.1 | Hidden Knowledge Poisoning Detection | 隐蔽污染检测 | LR1/R0/P0/L1 accepted；W2 not executed；P1 not started | `PLANNED` | [长期需求](long_term_research_requirements.md) | accepted W2 evidence then separate P1 decision |
...
| BLK-S6.1-FU1-W2-001 | 2026-07-31 | Worker validation | high | GMTP executable core | `OPEN` | hardened exact W2 contract | no downgrade/reimplementation | accepted W2 evidence | await owner execution decision |
...
`READY_FOR_OWNER_EXECUTION_APPROVAL / NOT_YET_EXECUTED`。S6.1-P1、Dataset、Detector、training and formal RAG experiment

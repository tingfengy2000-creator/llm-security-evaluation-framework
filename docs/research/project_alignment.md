# CodeGuarder 与教育厅立项项目对齐说明

## 依据与边界

本说明依据《面向检索增强生成系统的隐蔽知识污染检测与可信检索关键技术研究》立项申报书整理。该申报书是研究路线依据，不是 CodeGuarder 已完成成果证明；文档不复述申报书中的个人信息、联系方式或单位信息。

立项项目计划完成时间为 2028 年 12 月，研究主线为：隐蔽污染威胁建模、中文基准构建、多视角检测、风险感知可信检索、鲁棒证据聚合和原型系统评测。

## 对齐矩阵

| 立项研究内容 | CodeGuarder 对应模块 | 当前状态 | 后续成果边界 |
| --- | --- | --- | --- |
...
```text
立项：威胁数据建模
  → CodeGuarder：R1–R6、R5 隐蔽污染子类型、CorpusSnapshot
立项：多视角检测
  → CodeGuarder：EvidenceSignal 与 no-label-leakage
立项：可信检索与鲁棒聚合
  → CodeGuarder：TrustAggregator + RetrievalPolicy
立项：原型与综合验证
  → CodeGuarder：RAG Runner + E0–E5 实验轨道 + 审计报告
```
...
CodeGuarder 尚未完成中文隐蔽污染基准、联合检测模型、可信重排、鲁棒聚合、跨数据集迁移、真实大规模评测或立项预期论文/软著成果。现有 Stage 1–5 是立项书所述“实验与工程基础”的补充证据，不等同于申报书中团队全部既有成果。

...
| 当前任务 | `PILOT4-A-B-HUMAN-ANNOTATION-USABILITY-REPAIR-01`：在不改协议、Candidate、ID、顺序或 schema 的前提下，把 A/B Phase1 整理为可安全分发的人类可用 V2。 |
| 当前审批门 | Pilot4 Protocol 已接受、calibration 已关闭且 A/B execution 已批准；V1 未分发并仅因 human usability 被 V2 替代，A/B Phase1 V2 均 ready，Phase2 仍 withheld。 |
| 下一批准任务 | Owner 分别向 HUMAN-A01/HUMAN-B01 发放各自六个 Phase1 V2 文件；A/B annotation 尚未执行，240-group、Dataset、Formal Detector、Training 和 Formal Experiment 均未启动。 |
...
| 当前主要阻塞项 | 无协议设计或校准 systemic blocker；开放 gate 是 `A_B_EXECUTION_APPROVAL_AND_ROSTER_PENDING`。Formal A/B、Scale Pilot、Dataset 和 method effectiveness 均未冻结。 |
...
| Stage 6.1 | Hidden Knowledge Poisoning Detection | 隐蔽污染检测 | LR1/R0/FU1 accepted；P1-R1 framework accepted；Pilot4 protocol accepted/calibration closed；A/B execution approved；human Phase1 V2 ready；formal work not started | `PILOT_IN_PROGRESS / WAITING_OWNER_PHASE1_V2_DISTRIBUTION / NOT_FORMAL_EXPERIMENT` | [长期需求](long_term_research_requirements.md) | Owner separately distributes each annotator's six V2 Phase1 files；Phase2 remains withheld |
